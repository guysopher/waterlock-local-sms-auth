...
exports.getHtmlEmail = function(token){
  var config = require('./waterlock-phone-sms-auth').config;
...
exports.checkSmsCodes = function(smsCode, auth) {
  ...
  var testAccount = require('./waterlock-phone-sms-auth').testAccount;
  
  if (auth.phone == testAccount.phone) {
    if (smsCode == testAccount.smsCode) {
      return {
        success: true
      }
    } else {
      return {
        err: "Test account phone but SMS code do not match"
      }
    }
  } else {
    var bcrypt = require('bcrypt');
...
exports.sendSmsCode = function(phoneNumber, smsCode) {
  ...
  var config = require('./waterlock-phone-sms-auth').sms;
